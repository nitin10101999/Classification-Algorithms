...
Original file is located at
    https://colab.research.google.com/drive/162g8-qzTId4kA5Ub30UixRbBcY6h-oaX
"""
...
def testModel(P_Ci_s,Means,CovMat,X_test,y_test):
  ...
  for i in range(len(X_test)):
    ...
    for j in range(len(P_Ci_s)):
      temp = multivariate_normal.pdf(X_test[i], Means[j],CovMat[j],allow_singular=True)* P_Ci_s[j]
      if temp > max:
        max = temp
        mx_index = j
...
def bayesFilter(x,y,k):
  ...
  for label in labels:
    ...
    CovMat.append(((np.transpose(Zi)).dot(Zi))*(1/ni))
...
P_Ci_s,Means,CovMat = bayesFilter(x,y,k=2)
#print(CovMat[0])
